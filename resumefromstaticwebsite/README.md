...
This project also includes a CloudFormation template, [`oneclicksetup.yaml`](./oneclicksetup.yaml), that automates the setup above. It takes input parameters and provisions the S3 bucket (configured for static website hosting) and the CloudFront distribution (with HTTPS) in one deploy, instead of clicking through the console manually.

Note: the template only provisions the infrastructure. After the stack is created, you'll need to upload your own `resume.html` and `resume.css` (or any other static site files) to the newly created S3 bucket yourself.
...
| `oneclicksetup.yaml` | CloudFormation template that provisions the S3 bucket and CloudFront distribution |
